"""This is the module to run the INCREMENT turing machine program"""
import getopt
import sys
...
def main(argv):
    ...
    try:
        ...
        print('usage: main_increment.py -i <inputfile>')

        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('main_increment.py -i <inputfile>')
            sys.exit()
        elif opt in ("-i", "--ifile"):
            inputfile = arg

    program = open("tm_INCREMENT.txt").read()
